...
void UART_Receive_Processing()
{
    ...
    if(Uart_Pretreatment()) {
        ...
        //WiFi模组获取设备信息
        case 0x01:
            //Data_Encapsulation(Device_information, sizeof(Device_information), 0x00, Sn, 0x00);
            break;
...
//                MCU_Application_Control(RX1_Buffer);
                Data_Encapsulation(9, 0x04, Sn, 0x00);
                //MCU主动上报当前状态
                Sn = Sn + 0x01;
                Data_Encapsulation(14, 0x05, Sn, 0x04);
                MCU_OLED_StatusChange(RX1_Buffer[9]);
            }
            // WiFi模 组 读 取 设 备 的 当 前 状 态
            if(RX1_Buffer[8] == 0x02) {
                Data_Encapsulation(14, 0x04, Sn, 0x03);
            }
            break;
        //MCU主动上报当前状态WIFI模组回复
        case 0x06:
        ...
        case 0x07:
            ...
            Data_Encapsulation(9, 0x08, Sn, 0x00);
            //打开定时器
            TR0 = 1;
            break;
        ...
        case 0x0d:
            ...
            Data_Encapsulation(9, 0x0e, Sn, 0x00);
            break;
        case 0x0f:
            Data_Encapsulation(9, 0x10, Sn, 0x00);
            break;
        //WiFi模组非法数据包通知
        case 0x11:
            Data_Encapsulation(10, 0x12, Sn, RX1_Buffer[8]);
            break;
...
void UART_Send_Processing()
{
    ...
    if(flag != 0 && Wifi_flag == 1) {
        ...
        case 0x01:
            ...
            Data_Encapsulation(10, 0x09, Sn, 0x01);
            break;
        //  配置WiFi模式(AP模式) 
        case 0x02:
            OLED_P6x8Str(2, 2, "Configure Wifi:AP");
            Data_Encapsulation(10, 0x09, Sn, 0x02);
            break;
        //  重置WiFi信息 
        case 0x03:
            OLED_P6x8Str(2, 2, "Reset Wifi!");
            Data_Encapsulation(9, 0x0b, Sn, 0x00);
            break;
        //  WiFi进入可绑定模式 
        case 0x04:
            OLED_P6x8Str(2, 2, "Enter bindable mode!");
            Data_Encapsulation(9, 0x15, Sn, 0x00);
            break;
        //   重启WiFi模组 
        case 0x05:
            OLED_P6x8Str(2, 2, "Restart Wifi!");
            Data_Encapsulation(9, 0x29, Sn, 0x00);
            break;
        }
